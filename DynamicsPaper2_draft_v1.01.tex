...
Several approaches to constrain an axisymmetric potential and/or orbit DF have been recently put forward: \citet{2013ApJ...779..115B} and \citet{2014MNRAS.445.3133P} fitted potential and DF simultaneously to stellar positions and velocities in the disk and got precise constraints on the overall potential; \citet{2015MNRAS.449.3479S} and \citet{2016MNRAS.tmp..817D} investigated extended DFs for the disk and halo respectively (given a fiducial potential), that extend the distribution in orbit space by an additional dimension, the metallicity of the star. 

In this work we will continue our investigation of the \RM{} approach (\emph{``Recovery of the Orbit Action Distribution of Mono-Abundance Populations and Potential INference for our Galaxy''}), which was first used by \citet{2013ApJ...779..115B} and in detail characterized by \citet{Trick 2016}, hereafter Paper I, in anticipation of the Gaia data release. Here we will specifically investigate the important question, if axisymmetric \RM{} modelling can still give reliable constraints on the potential in the presence of non-axisymmetric spiral arms in the data.


\begin{itemize}
\item The first application of \RM{} was conducted by \citet{2013ApJ...779..115B}, who measured the Milky Way's surface density profile within $1.1~\text{kpc}$ from stellar mono-abundance populations (MAPs) in the Galactic disk. They fitted a single qDF and potential with two \Wilma{Check} free parameters (disk scale length and halo contribution to the radial force at the solar radius) to stars from the \Wilma{?????} survey that had the same $[\mathrm{Fe}/\mathrm{H}]$ and $[\alpha/\mathrm{H}]$ \Wilma{Check}. For each of those 42 \Wilma{Check} different MAPs they then derived the radius for which the surface density was best constrained, such constructing a surface density profile that was, for $R>6~\text{kpc}$ \Wilma{Check}, later confirmed by \citet{2014MNRAS.445.3133P} using a different action-based procedure.
\item \Wilma{Trick et al. 2016}, hereafter Paper I, then studied the strengths and breakdowns of the \RM{} method in detail, by investigating a large suite of mock data sets. Under the prerequisite of axisymmetric data and model, they found that \RM{}'s modelling success is stable against minor misjudgements of distribution function or selection function, and that---if the true potential is not contained in the proposed family of model potentials---one can still find a good fit, given the limitations of the model. Paper I also found that measurement uncertainties of the order of those by the final Gaia data release should be good enough (within $3~\text{kpc}$) to allow for precise and unbiased modelling results. What was not investigated in Paper I, was the breakdown of axisymmetry in the data and what would happen if both axisymmetric distribution function and potential models would therefore not contain the true DF and potential anymore. This will be this work's subject of investigation.
