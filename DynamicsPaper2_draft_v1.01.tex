...
%TITLE
%-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
\title{The Influence of Spiral Arms on Action-based Dynamical Modelling for the Milky Way Disk\\}
...
\begin{abstract}
\RM{} is a well-tested and robust dynamical modelling machinery developed to constrain the Milky Way's (MW) gravitational potential by simultaneously fitting an axisymmetric parametrized potential model and an axisymmetric action-based stellar orbit distribution function (DF) to the discrete 6D phase-space measurements from stars in the Galactic disk. In this work we investigate if \RM{}'s robustness is still given for data affected by non-axisymmetric spiral arms for which the axisymmetric modelling cannot account. We do this by drawing data from a N-body simulation snapshot of a MW-like spiral galaxy. We find that \RM{} gives surprisingly successful potential constraints even in the presence of strong spiral arms and even when using the most simple assumption for an action-based DF, the quasi-isothermal DF (qDF). For data from large survey volumes \RM{} finds models that average successfully over the spiral arms. As was to be expected the recovered models are slightly biased by the excess of stars in the spiral arms that feel stronger vertical gravitational forces. For all and even small survey volumes with radius $r_\text{max}>500~\text{kpc}$ around the Sun the best fit model always reproduces the forces at the positions of the stars that entered the analysis very well. Survey volumes with $r_\text{max}\leq 3~\text{kpc}$ give already predictions for the \emph{overall} gravitational potential as good as those from larger volumes. However, only volumes as large as $r_\text{max}=5~\text{kpc}$ are able to recover the correct dark matter halo scale length. While small volumes ($r_\text{max} < 2~\text{kpc}$) centered on spiral arms do not have a large predictive power, inter-arm regions appear smooth and axisymmetric enough such that the potentials derived from small volumes centered here can be extrapolated to regions outside of the survey volume more reliably. The stellar phase-space measurements from the upcoming Gaia data releases will cover a large proportion of the MW disk. As all survey volumes give good potential constraints where most of the stars are located, \RM{} should not only be able to find a good average potential model for the MW from Gaia data. By breaking up and modelling the data in smaller volumes independently, \RM{} will also be able to make correct measurements of non-axisymmetric structures in the MW's gravitational potential. \Wilma{[TO DO: Too long. Cut out 110 words.]}
\end{abstract}
...
\subsection{Recovering non-axisymmetric structures from the potential from small volumes}
