...
\end{equation}
with $R_s = 2.5~\text{kpc}$ and $z_0=0.1R_s$ and total disk mass $M_* = 0.04\cdot M_\text{dm} = 3.8\cdot 10^{10} M_\odot$. \Elena{[TO DO: Elena wants me to add here: "In this model the disk fraction within 2.2 x scale lengths is 50\% of the total mass, leading to a formation of approximately four arms (D'Onghia 2015, ApJ, 808, L8)."]}
...
\Jo{[TO DO: Jo writes: "I know that we don’t want to change anything major, but I think it would be nice if there is *some* reason for focusing on the rmax=4 kpc volume. For example, is it where we can expect good data for Gaia DR2, DRX, final DR? If so, that would be a good reason to focus on this volume. Is there isn’t a simple reason like this, then just ignore this comment, but think it would help if there was a simple reason like this."]}
...
\end{eqnarray}
\Jo{[TO DO: Jo writes: "Equation (23): I’m confused about what this is supposed to show? Maybe just remove it (hope I didn’t suggest adding this!)? I think just using $z=h_z$ makes sense without any reasoning."]} where, e.g., $\langle |F_{z,T}(*_i)|\rangle (R) $ denotes the mean of the absolute value of the true vertical force of all stars at radius $R$, and $J_z \times \Omega_z \sim \langle E_z \rangle$ is close to the mean vertical energy of a stellar orbit. Figure \ref{fig:4kpc8Spiral_forces} shows therefore
\begin{eqnarray}
...
\HW{[TO DO: HW wants me to add: "If we model actual Milky Way data, and have independent information on the location and strength of spiral arms, we could use the present approach to
