...
The disk consists of $10^8$ ``disk star'' particles, each having a mass of $\sim370 ~M_\odot$, and 1000 ``giant molecular cloud'' particles with mass $\sim9.5\cdot 10^{5} ~M_\odot$. Initially the particles are distributed following an exponential disk profile with density \Elena{[TO DO: Elena writes: "It is more accurate to write something like: "The vertical mass distribution of the stars in the disk is specified by the profile of an isothermal sheet with a radially constant scale height z0. The scale-length Rs is assumed to be 2.5 kpc and z0=0.1......."]}
\begin{equation}
\rho_*(R,z) = \frac{M_*}{4\pi z_0 R_s^2} \text{sech}^2 \left( \frac{z}{z_0}\right) \exp \left(- \frac{R}{R_s} \right), \label{eq:sech_disk}
\end{equation}
with $R_s = 2.5~\text{kpc}$ and $z_0=0.1R_s$ and total disk mass $M_* = 0.04\cdot M_\text{dm} = 3.8\cdot 10^{10} M_\odot$. \Elena{[TO DO: Elena wants me to add here: "In this model the disk fraction within 2.2. scale lengths is 50\% of the total mass, leading to a formation of approximately four arms (D'Onghia 2015, ApJ, 808, L8)."]}
...
\Elena{[TO DO: Elena writes: "The model I gave you is the one in D'Onghia et al. 2013 (Fig.8 left top panel) and D'Onghia 2015. It has a disk fraction of  50\% at 2.2. scale lengths (I wrote you a comment on that in the draft). Note that this is **not** what has been used in Vera-Ciro, D'Onghia \& Navarro 2016. The ones in Vera-Ciro et al. have a different rotation curve and different structural parameters. By changing the structural parameters you change the spiral structure. This addresses the concerns of Ivan. ALSO, It is not neither the MULTI-ARMED flocculent galaxy mostly analyzed in D'Onghia et al. 2013."]}
...
\end{eqnarray}
where $v_{\text{circ},S}$ and $F_{z,S}$ are the circular velocity and vertical force evaluated in the ``true symmetric'' reference potential \texttt{DEHH-Pot} in Table \ref{tbl:DEHH-Pot}. As a typical vertical force at a given radius we use the vertical force evaluated at the scale height $h_z=0.17~\text{kpc}$ of the disk, which is motivated by our findings that 
\begin{eqnarray} 
...
\Elena{[TO DO: Elena finds this title odd.]}
...
ED gratefully acknowledges the support of the Alfred P. Sloan Foundation and ATP NASA Grant No NNX144AP53G. 
