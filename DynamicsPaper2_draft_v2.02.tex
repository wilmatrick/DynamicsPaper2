...
Secondly, we note \NEW{again} that \OLD{$F_z(g_j)$}\NEW{$F_z(\vect{x}_{g,j})$} is \OLD{recovered much worse}\NEW{predicted less good} than \OLD{$F_R(g_j)$}\NEW{$F_R(\vect{x}_{g,j})$}. The reason for this \OLD{was already}\NEW{is} laid out in Section \ref{sec:forces_bias}). 
