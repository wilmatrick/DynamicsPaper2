...
\section{\NEW{Calibrating the method with modelling a snapshot without spiral arms}} \label{sec:MNdHHinit}

\NEW{To better understand the effect of spiral arms on the \RM{} modeling in the previous sections, we also performed a calibration test run. For that we applied \RM{} to a mock data set drawn from the same spherical survey volume as in Section \ref{sec:results_part1} (centered at \texttt{S8} with maximum radius $r_\text{max}=4~\text{kpc}$), but from the initial axisymmetric snapshot of the galaxy simulation, in which no spiral arms had evolved yet.}
...
\NEW{The recovered potential parameters in Table \ref{tbl:MNHHinit_4kpc8Spiral}, $a_\text{disk}=(3.73 \pm 0.04) ~\text{kpc}$, $b_\text{disk}=(0.34 \pm 0.03) ~\text{kpc}$, $f_\text{halo}=(0.52 \pm 0.03)$, and $a_\text{halo}=(24 \pm 2) ~\text{kpc}$, are consistent with the spiral arm-affected measurements in Table \ref{tbl:MNHHdiffSph2_4kpc8Spiral} to within 2, 4, 1, and 2 times the statistical error, respectively. While the halo scale length $a_\text{halo}=(24 \pm 2)~\text{kpc}$ in this calibration run is also consistent with the truth within 3 times the error, we expect it to be underestimated to account for the Miyamoto-Nagai disk being too massive at large radii. Compared to the truth, the circular velocity curve is underestimated by $\sim2.5\%$; $v_\text{circ}(R_\odot)=(219.36 \pm 0.08) ~\text{km s}^{-1}$. This might be related to the slightly unphysical setup of the $v_T$ velocities in the initial snapshot, but could also be a systematic bias due to the chosen model family. Because the simulation immediately develops spiral arms after the initial snapshot, there was no snapshot, being both still axisymmetric and already in a dynamical steady state, to resolve this further.}
