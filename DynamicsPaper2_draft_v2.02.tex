...
\NEW{To better understand potential biases in the modeling, we have also performed an analogous \RM{} analysis of the initial, still axisymmetric simulation snapshot in Appendix \ref{app:MNdHHinit}.}
...
\section{\NEW{Recovering potential and DF in a simulation without spiral arms}} \label{app:MNdHHinit}

%====================
\begin{deluxetable}{ll}[!htbp]
\tabletypesize{\scriptsize}
\tablecaption{Best fit \texttt{MNHH-Pot} as recovered from the \RM{} analysis of data drawn from the initial axisymmetric simulation snapshot and a survey volume with $r_\text{max}=4~\text{kpc}$ centered on position \texttt{S8}. \label{tbl:MNHHinit_4kpc8Spiral}}
\tablewidth{0pt}
\startdata
\tableline
$v_\text{circ}(R_\odot)$ & $(219.36 \pm 0.08) ~\text{km s}^{-1}$\\
$a_\text{disk}$ & $(3.73 \pm 0.04) ~\text{kpc}$\\
$b_\text{disk}$ & $(0.34 \pm 0.03) ~\text{kpc}$\\
$f_\text{halo}$ & $(0.52 \pm 0.03)$\\
$a_\text{halo}$ & $(24 \pm 2) ~\text{kpc}$
\enddata
\end{deluxetable}
%====================

\NEW{To better understand the effect of spiral arms on the \RM{} modeling in Section \ref{sec:results_part1}, we performed a calibration test run. For that we applied \RM{} to a mock data set drawn from the same spherical survey volume as in Section \ref{sec:results_part1} (centered at \texttt{S8} with maximum radius $r_\text{max}=4~\text{kpc}$), but from the initial axisymmetric snapshot of the galaxy simulation, in which no spiral arms had evolved yet. The spatial tracer distribution was very well recovered by \RM{}, as well as the distribution of tracers in $v_R$ and $v_z$  (see Figure \ref{fig:4kpc8Spiral_DF_comparison} for the analogous investigation for spiral arms). There were some deviations in the $v_T$ distribution however, with our model predicting more asymmetric drift. We accounted this to the simple initial setup of the disk's velocity distribution, which does not follow a physical distribution function but only a triaxial Gaussian \citep{2005MNRAS.361..776S}. We expect the modeling in this calibration run to be slightly biased by this.}

\NEW{The recovered potential parameters in Table \ref{tbl:MNHHinit_4kpc8Spiral}, $a_\text{disk}$, $b_\text{disk}$, $f_\text{halo}$, and $a_\text{halo}$, are consistent with the spiral arm-affected measurements in Table \ref{tbl:MNHHdiffSph2_4kpc8Spiral} to within 2, 4, 1, and 2 times the statistical error, respectively. While the halo scale length $a_\text{halo}=(24 \pm 2)~\text{kpc}$ in this calibration run is also consistent with the truth within 3 times the error, we expect it to be underestimated to account for the Miyamoto-Nagai disk being too massive at large radii. Compared to the truth, the circular velocity curve is underestimated by $\sim2.5\%$. This might be related to the slightly unphysical setup of the $v_T$ velocities in the initial snapshot, but could also be a systematic bias due to the chosen model family. Because the simulation immediately develops spiral arms after the initial snapshot, we could not resolve this further by investigating a different snapshot, which is both still axisymmetric and already in a steady state.}
